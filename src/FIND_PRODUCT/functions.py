...
def canonical_hash_component(comp, adj, elements):
    """
    Canonical hash of ONE connected component.
    comp: list of atom indices (e.g. [0,1,2])
    """
    comp = list(comp)
    sub_adj = adj[np.ix_(comp, comp)]
    sub_elems = [elements[i] for i in comp]

    pairs = [
        (sub_elems[i], sorted(sub_adj[i].tolist()))
        for i in range(len(comp))
    ]
    return hash(str(sorted(pairs)))


def canonical_hash_product(components, adj, elements):
    """
    Canonical hash of the whole product (multiple components).
    """
    comp_hashes = [
        canonical_hash_component(comp, adj, elements)
        for comp in components
    ]
    return hash(str(sorted(comp_hashes)))
...
def build_product_index_and_write_files(
    ...
    """
    Parse trajectory file, detect chemical products via connected components,
    canonicalize product graphs, write xyz files per trajectory,
    and build product_index including canonical forms.
    """

    natoms = len(elements)
    traject = parse_trajectory_file(trajectory_file, natoms=natoms)

    # List of unique trajectory numbers
    ...
    for trj_no in traj_numbers:
        ...
        with open(outname, "w") as outfile:
            ...
            current_product = None           # canonical hash of current product
            current_comp_norm = None         # normalized original components
            start_cycle = None
            start_time = None
            Q_last = None

            # ==========================================================
            # Loop over cycle counts
            # ==========================================================
            for cycle in cycle_counts:

                # Only sample every N cycles
                if cycle % print_interval != 0:
                    continue

                entry = next(e for e in traj_entries if e["cycle_count"] == cycle)
                Q = np.array(entry["Q"])
                time = entry["time"] * 10.0  # convert Venus to fs
                ...
                canonical = canonical_hash_product(components, adj, elements)

                # Product changed?
                if canonical != current_product:

                    # Close previous product block
                    if current_product is not None:
                        ...
                        product_index[trj_no].append({
                            "canonical": current_product,
                            "components": current_comp_norm,
                        ...
                        # Write XYZ block
                        header = (f"canonical {current_product} | "
                                  f"cycle {start_cycle} to {end_cycle} | "
                                  f"duration = {duration:.6f} fs")
                        write_xyz_block(outfile, Q_last, elements, header)

                    # Start new product
                    current_product = canonical
                    current_comp_norm = comp_norm
                    start_cycle = cycle
                    start_time = time

                # Update last geometry
                Q_last = Q.copy()

            # ==========================================================
            # After the loop, close final product
            # ==========================================================
            if current_product is not None:
                end_cycle = cycle_counts[-1]
                end_time = next(e["time"] for e in traj_entries
                                if e["cycle_count"] == end_cycle) * 10.0
                ...
                header = (f"Final canonical {current_product} | "
                          f"cycle {start_cycle} to {end_cycle} | "
                          f"duration = {duration:.6f} fs")
                write_xyz_block(outfile, Q_last, elements, header)
#        product_index[trj_no] = merge_product_blocks(product_index[trj_no])


    return product_index
def merge_product_blocks(blocks):
    merged = {}
    for b in blocks:
        key = b["canonical"]
        ...
        if key not in merged:
            merged[key] = {
                ...
                "intervals": [(b["start_cycle"], b["end_cycle"])],
                "start_time": b["start_time"],
                "end_time": b["end_time"]
            }
        else:
            merged[key]["total_duration"] += duration
            merged[key]["intervals"].append((b["start_cycle"], b["end_cycle"]))
            merged[key]["end_time"] = b["end_time"]
    return list(merged.values())

# ----------------- Search across product_index -----------------
def search_products_file_across_trajectories(products_xyz_file,
                                             product_index, elements, C=1.3):
    ...
    for idx, (elems_frame, coords) in enumerate(frames, start=1):
        ...
        # NEW: compute canonical hash of the product geometry in the xyz file
        canonical_query = canonical_hash_product(components, adj, elements)
        matches = []
        for trj_no, products in product_index.items():
#            for prod in products:
#                if prod["canonical"] == canonical_query:
...
#                 print(last_prod)
                 ...
                 if last_prod["canonical"] == canonical_query:
                    duration = last_prod["end_time"] - last_prod["start_time"]
                    matches.append({
                    "traj_no": trj_no,
                    "duration": duration
                     })
#                    print(idx)
#                    print(trj_no, products)
                    key=False

        # Store result
        results[idx] = {
            "components": comp_norm,
...
#                if m['duration'] >= 50.00:
                ...
                #print("canonical:", canonical_query)
                #print(f"instances: {len(matches)}")
                #print(f"cycles {m['start_cycle']} -> {m['end_cycle']}")
                #print(f"time {m['start_time']:.6f} -> {m['end_time']:.6f}")
                #print(f"duration = {m['duration']:.2f}\n")
#        else:
            #print("0")
#            print("canonical:", canonical_query)
#            print("Not found.\n")
